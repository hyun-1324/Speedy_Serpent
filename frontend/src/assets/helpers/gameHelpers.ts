...
export const isLethalCollision = (
  collision: any
): collision is LethalCollision => {
  return collision.type && 'collision' in collision.type;
};

// Type guard to check if collision is a ResourceCollision
export const isResourceCollision = (
  collision: any
): collision is ResourceCollision => {
...
export const moveSegment = (
  ...
  if (
    previousSegment.x === targetSegment.x &&
    previousSegment.y === targetSegment.y
  ) {
...
export const handleLethalCollision = (
  ...
  switch (cause) {
    case 'wall':
      message = isMe
        ? `Your snake collided with ${CollisionCause.Wall} and died!`
        : `${collision.playerName}'s snake collided with ${CollisionCause.Wall} and died!`;
      break;
    case 'self':
      message = isMe
        ? `Your snake collided with ${CollisionCause.Self} and died!`
        : `${collision.playerName}'s snake collided with ${CollisionCause.Self} and died!`;
      break;
    case 'snake':
      message = isMe
        ? `Your snake collided with ${CollisionCause.Snake} and died!`
        : `${collision.playerName}'s snake collided with ${CollisionCause.Snake} and died!`;
      break;
    default:
      console.error('Unknown collision type when handling lethal collision');
  }
  const color = getPlayercolor(collision.playerName);
  const colorClass: `${string}Text` | undefined = color
    ? `${color}Text`
    : undefined;
