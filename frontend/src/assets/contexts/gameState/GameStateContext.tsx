...
import { ProviderProps, GameStateContextProps, GameState, ViewState, AudioState, GameEventMessage, Timer } from "../../types/interfaces";
...
export const GameStateProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  const [gameEventMessages, setGameEventMessages] = useState<GameEventMessage[]>([]);
  const [timer, setTimer] = useState<Timer>({roundDuration: 120, timeLeft:"02:00"});
  const [viewState, setViewState] = useState<ViewState>({status: "loading", showInfo: false});
  ...
  const getPlayercolor = (playerName: string) : string | undefined => {
    return gameState.players.filter((player) => player.name === playerName)[0].color;
  }

  useEffect(() => {
    // Listen for game state updates from the server
    listenToPlayerEvents({ socket, setGameState, setGameEventMessages });
    listenToGameStateEvents({ socket, setGameState, setViewState, setTimer, setGameEventMessages });
    listenToErrors({ socket, setGameState, setViewState, setGameEventMessages });
